...
echo "📦 [2/5] Архівую том DB ($VOL_DB)..."
docker run --rm \
  -v "$VOL_DB":/volume \
  -v "$BACKUP_DIR":/backup \
  alpine sh -c "cd /volume && tar -czf /backup/mariadb_volume.tar.gz ." || echo "⚠️ Том $VOL_DB не знайдено, пропускаю."

# === 3. Бекап томів Koha (Data & Config) ===
echo "📦 [3/5] Архівую томи Koha (Config, Data)..."
...
# === 4. Бекап логів (НОВИЙ КРОК) ===
echo "📜 [4/5] Архівую логи Koha..."
docker run --rm \
  -v "$VOL_LOGS":/volume \
  -v "$BACKUP_DIR":/backup \
  alpine sh -c "cd /volume && tar -czf /backup/koha_logs.tar.gz ." || echo "⚠️ Том $VOL_LOGS не знайдено."

# === 5. Бекап тома Elasticsearch ===
echo "📦 [5/5] Архівую том Elasticsearch..."
docker run --rm \
  -v "$VOL_ES":/volume \
  -v "$BACKUP_DIR":/backup \
